...
'my_physeq' is now a new phyloseq object which cotains the data (OTU table, sample data, and taxonomy table) that we just imported.
...
The distance table records the Bray_Curtice distance between all samples.
...
Below is the code to color the samples based on their catagory in SampleOrigin.
...
In that case we can accept the alternative hypothesis which means that the diversity is significantly different between Freshwater and Human samples.

# 6. Beta diversity

```{r}
#dist = phyloseq::distance(my_physeq, method="jaccard")
